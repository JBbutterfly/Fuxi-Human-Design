...
import { birthDataToUtcDate, type LocalChartInput } from "@/lib/localChart";
import {
  CENTER_NAMES,
  CENTER_DESCRIPTIONS,
  PROFILE_NARRATIVES,
...
export interface ChartReportProps {
  input: LocalChartInput;
  /** Hide the "New chart" shortcut — e.g. when embedded read-only on the community page. */
  showNewChartLink?: boolean;
}

export function ChartReport({ input: stored, showNewChartLink = true }: ChartReportProps) {
  const chart = useMemo<HdChart>(() => computeChartFromUtcParts(birthDataToUtcDate(stored.birthData)), [stored]);
  const astro = useMemo(
    () => computeNatalAstrology(chart.personalityTime, stored.birthData.latitude, stored.birthData.longitude),
    [chart, stored],
  );
  const svg = useMemo(() => renderBodyGraphSvg(chart, { title: stored.subjectName }), [chart, stored]);
